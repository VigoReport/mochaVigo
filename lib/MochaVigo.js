  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
        ...
        obj.stats.skipPer = pendingPercentage
        ...
        if (reporterOpts['project-key']) {
          ...
          var os = Utils.getOsInfo()
          let sdk = Utils.getSDKinfo()

          let finalObj = {
            build: Object.assign(obj.stats, { projectKey: reporterOpts['project-key'] }, { os: os }, { sdk: sdk }),
            suites: obj.suites.suites
          }

          Utils.handShakeWithVigo(finalObj)
          Utils.saveToFile(stringify(finalObj, null, 2), './testOutPut.json', function () {})
