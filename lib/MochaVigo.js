			...
			if (!endCalled) {
				...
				async.auto({
						...
						copyFile: ['createDirs', function (cb) {
							Utils.copyFile(config.srcHtml, config.reportDir + config.splitChar + 'index.html')
						...
						copyAssets: ['createDirs', function (cb) {
							Utils.copyFile(config.srcAssets, config.trgAssets + config.splitChar + 'main.js')
								.then(function (success) {
									cb()
								})
								.catch(function (err) {
				...
				if (config.autoOpen) {
					...
					opener(config.reportDir + config.splitChar + 'index.html')
				}
