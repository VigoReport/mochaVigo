...
module.exports = function (options) {
	...
	// Source Directories
	config.srcAssets = path.join(__dirname, '..', 'npmBuild' + config.splitChar + 'dist' + config.splitChar + 'main.js');
	config.srcHtml = path.join(__dirname, '..', 'npmBuild' + config.splitChar + 'index.html');

	config.trgAssets = path.join('.', config.reportDir + config.splitChar + 'dist');

	// change Json data in file path
	config.changeDataPath = path.join('.', config.reportDir + config.splitChar + 'dist' + config.splitChar + 'main.js')
	return config;
};
